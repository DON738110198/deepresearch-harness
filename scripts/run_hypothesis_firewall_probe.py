...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a preregistered Hypothesis Firewall retrieval-slate probe."
    )
    ...
    registration = load_firewall_registration(args.registration)
    ...
    if settings.provider.thinking_mode != registration.fixed_contract.thinking_mode:
        parser.error("provider thinking mode differs from the firewall registration")
