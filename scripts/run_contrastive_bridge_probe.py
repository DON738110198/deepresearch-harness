...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a preregistered contrastive bridge-hypothesis probe.")
    parser.add_argument("--registration", type=Path, required=True)
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--search-timeout-seconds", type=int, default=60)
    args = parser.parse_args()
    registration = load_bridge_registration(args.registration)
    ...
    if settings.provider.thinking_mode != registration.fixed_contract.thinking_mode:
        parser.error("provider thinking mode differs from the bridge registration")
    ...
    print(f"oracle_gold_hit_cases={result.oracle_gold_hit_cases}")
