...
from deepresearch_harness.corpus_bridge_induction import (
    load_corpus_bridge_registration,
    run_corpus_bridge_probe,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a preregistered corpus-grounded bridge induction probe."
    )
    ...
    registration = load_corpus_bridge_registration(args.registration)
    ...
    if settings.provider.thinking_mode != registration.fixed_contract.thinking_mode:
        parser.error("provider thinking mode differs from corpus bridge registration")
    provider = provider_from_config(settings)
    result = run_corpus_bridge_probe(
    ...
    print(f"prior_search_calls={result.prior_search_calls}")
    print(f"final_search_calls={result.final_search_calls}")
    print(f"cumulative_search_calls={result.cumulative_search_calls}")
    print(f"parse_failures={result.parse_failures}")
    print(f"provider_request_failures={result.provider_request_failures}")
