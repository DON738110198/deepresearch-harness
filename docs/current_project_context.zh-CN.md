...
- 尚未实现：hash-bound paired search executor（已可离线注册双臂、尚未执行或恢复）、来源质量分层、迭代补搜/重规划、自动语义评测、Research DAG、多 Agent 和 Critic-Repair。
...
- Fresh public 入口：LiveDRBench 已固定未使用 keys `[10,23,38,86,99]`。baseline 是既有 live collector；candidate 是 Tavily basic search adapter。现在可离线冻结两臂 config snapshot 与 `pair_manifest.json`，但尚无 executor、生成、官方 Judge 或效果数字。搜索 attempt 硬上限为 5；这个比较只 token-match，工具费用必须单列，不能冒充 cost-match。
